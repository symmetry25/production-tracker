...
export default async function ReviewPage({ params }: ReviewPageProps) {
  ...
  const workspace = await getReviewVersionWorkspace(versionId);
      ...
      <div className="mb-4 flex items-end justify-between gap-5">
        <div>
          ...
          <p className="mt-2 max-w-3xl text-sm leading-6 text-[#aaa599]">
            {selected.task.contextLabel} / {selected.task.name} · {selected.uploadedBy.name} · {selected.status}
          </p>
        </div>
      </div>

      <ReviewWorkspace versions={versions} initialVersionId={selected.id} />
    </>
  );
}
