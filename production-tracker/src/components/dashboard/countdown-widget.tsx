...
export function CountdownWidget({
  ...
  project: DashboardStats["project"];
  ...
  return (
    <div className="grid gap-4 xl:grid-cols-[1.1fr_1.4fr]">
      <div className="border border-[#34322b] bg-[#181713] p-5">
        <p className="text-xs font-semibold uppercase tracking-[0.28em] text-[#d8b46a]">{labels.eyebrow}</p>
        <div className="mt-6 flex items-end gap-3">
          <span className={["font-mono text-6xl font-semibold leading-none", tone].join(" ")}>{project.daysRemaining}</span>
          <span className="pb-2 text-sm text-[#aaa599]">{labels.daysToDueDate}</span>
        </div>
        <p className="mt-4 text-sm text-[#c9c3b5]">{project.name} · {project.code}</p>
        <p className="mt-1 text-xs text-[#8f8a7e]">{labels.due} {dueLabel}{project.milestone ? ` · ${project.milestone}` : ""}</p>
      ...
      <div className="grid grid-cols-5 border border-[#34322b] bg-[#181713]">
        <Metric label={labels.metrics.shots} value={counts.shots} />
        <Metric label={labels.metrics.assets} value={counts.assets} />
        <Metric label={labels.metrics.tasks} value={counts.tasks} />
        <Metric label={labels.metrics.versions} value={counts.versions} />
        <Metric label={labels.metrics.crew} value={counts.crew} />
      </div>
    </div>
  );
}
